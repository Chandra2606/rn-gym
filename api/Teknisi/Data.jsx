...
export default function Data() {
  ...
  //memanggil api
  const fetchDataTeknisi = async (pageNumber = 1, searchQuery = search) => {
    let token = await AsyncStorage.getItem('userToken');
    setLoading(true);
    setError('');
    try {
      const response = await fetch(
        `${apiUrl}teknisi?page=${pageNumber}&search=${searchQuery}`,
        {
          headers: {
      ...
      setDataTeknisi(
        pageNumber === 1 ? json.data : [...dataTeknisi, ...json.data],
      );
    } catch (error) {
  ...
  const onRefresh = () => {
    setRefreshing(true);
    fetchDataTeknisi(1, search).finally(() => setRefreshing(false));
  };
  ...
  const rederItemTeknisi = ({item}) => {
    const showDetailData = () => {
      navigation.navigate('DetailTeknisi', {kodeteknisi: item.kodeteknisi});
    };
    return (
      <TouchableOpacity style={styles.item} onPress={showDetailData}>
        <Text style={styles.title}>{item.nama_lengkap}</Text>
        <Text style={styles.nim}>{item.kodeteknisi}</Text>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        data={dataTeknisi}
        renderItem={rederItemTeknisi}
        keyExtractor={item => item.kodeteknisi}
        extraData={loading || error}
        onEndReached={() => {
          if (!loading && page < lastPage) {
            fetchDataTeknisi(page + 1);
          }
        }}
