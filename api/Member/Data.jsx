...
export default function Data() {
  ...
  const fetchdataMember = async (pageNumber = 1, searchQuery = search) => {
    ...
    try {
      ...
      setDataMember(
        pageNumber === 1 ? json.data : [...dataMember, ...json.data],
      );
    } catch (error) {
  ...
  const onRefresh = () => {
    setRefreshing(true);
    fetchdataMember(1, search).finally(() => setRefreshing(false));
  };
  ...
  const rederItemDosen = ({item}) => {
    const showDetailData = () => {
      navigation.navigate('DetailMember', {kdmember: item.kdmember});
    };
    return (
      <TouchableOpacity style={styles.item} onPress={showDetailData}>
        <Text style={styles.title}>{item.namamember}</Text>
        <Text style={styles.nim}>{item.kdmember}</Text>
      </TouchableOpacity>
    );
  };
